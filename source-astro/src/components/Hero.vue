<template>
  <div class="hero">
    <header class="d-flex align-items-center bg-dark p-5" :style="{ backgroundImage: `url(${bgImg})` }">
      <div class="offset-lg-1 text-shadow mt-3 py-5">
        <div class="text-light">
          <img class="img-fluid mb-3" :src="wordsImg" alt="codecamp words" />
          <h3 class="text-shadow mt-3">
            {{ event.date }}
            <span class="ms-md-3 me-md-3">|</span>
            {{ event.location }}
          </h3>
          <div class="cta mt-3" v-if="register">
            <a :href="register" target="_blank"  class="btn btn-lg btn-outline-info">
              <span class="me-2">Register Now</span>
              <small>
                <i class="mdi  mdi-chevron-right"></i>
              </small>
            </a>
          </div>
        </div>
      </div>
    </header>
    <div class="bg-theme-dark p-2 text-light">
      <div class="container">
        <div class="row">
          <div class="col">
            <p class="m-0">
              <small>
                Need help convincing your manager you should attend this conference? It's easy this conference is
                <b class="text-theme-info">FREE!</b>
              </small>
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { AppState } from '../AppState.js';
import wordsImg from '../assets/img/codecamp-words.png';
import bgImg from '../assets/img/codecamp-header-bg.png';

export default {
  setup() {
    return {
      wordsImg: wordsImg?.src ?? wordsImg,
      bgImg: bgImg?.src ?? bgImg
    }
  },
  computed: {
    event() {
      return AppState.event;
    },
    register(){
      return AppState.register
    }
  }
};
</script>

<style scoped>
header {
  background-position: center;
  background-size: cover;
  min-height: 87vh;
  font-size: 18pt;
}
</style>
